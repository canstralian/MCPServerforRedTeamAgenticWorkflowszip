...
function handleUpdateTarget(args) {
    const parsed = UpdateTargetSchema.safeParse(args);
    if (!parsed.success) {
        return { content: [{ type: 'text', text: `Invalid arguments: ${parsed.error.message}` }] };
    }
    const { targetId, ...updates } = parsed.data;
    const target = store.updateTarget(targetId, updates);
    if (!target) {
        return { content: [{ type: 'text', text: 'Target not found' }] };
    }
...
function handleAddVulnerability(args) {
...
function handleDeleteTarget(args) {
    const parsed = GetTargetSchema.safeParse(args);
    if (!parsed.success) {
        return { content: [{ type: 'text', text: `Invalid arguments: ${parsed.error.message}` }] };
    }
...
export function handleTargetTool(name, args) {
    switch (name) {
        case 'create_target':
