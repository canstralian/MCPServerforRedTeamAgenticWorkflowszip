...
function handleCreateOperation(args) {
...
function handleGetOperation(args) {
...
function handleUpdateOperation(args) {
    const parsed = UpdateOperationSchema.safeParse(args);
    if (!parsed.success) {
        return { content: [{ type: 'text', text: `Invalid arguments: ${parsed.error.message}` }] };
    }
    const { operationId, ...updates } = parsed.data;
    const operation = store.updateOperation(operationId, updates);
    if (!operation) {
        return { content: [{ type: 'text', text: 'Operation not found' }] };
    }
...
function handleStartOperation(args) {
    const parsed = GetOperationSchema.safeParse(args);
    if (!parsed.success) {
        return { content: [{ type: 'text', text: `Invalid arguments: ${parsed.error.message}` }] };
    }
...
function handleCompleteOperation(args) {
    const parsed = GetOperationSchema.safeParse(args);
    if (!parsed.success) {
        return { content: [{ type: 'text', text: `Invalid arguments: ${parsed.error.message}` }] };
    }
...
function handleDeleteOperation(args) {
    const parsed = GetOperationSchema.safeParse(args);
    if (!parsed.success) {
        return { content: [{ type: 'text', text: `Invalid arguments: ${parsed.error.message}` }] };
    }
...
export function handleOperationTool(name, args) {
    switch (name) {
...
//# sourceMappingURL=operation-tools.js.map